      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: isDark
                ? const [Color(0xFF8F1029), Color(0xFF4A0715)]
                : const [Color(0xFFFF4D62), Color(0xFFD90429)],
          ),
...
class EmptyRecentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      final r = 18.0 + i * 16.0;
      arcPaint.color = accent.withValues(alpha: 0.5 - i * 0.13);
      canvas.drawArc(
        Rect.fromCenter(
...
class EmptyRecentState extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final accent = isDark ? AppTheme.primaryRed : AppTheme.lightAccent;
        ...
        child: Row(
          children: [
            ...
            AnimatedContainer(
              ...
              curve: Curves.easeOutCubic,
              transform: Matrix4.identity()
                ..translateByDouble(_pressed ? 4.0 : 0.0, 0.0, 0.0, 1.0),
              child: Icon(
...
class _PremiumSquareTileState extends State<PremiumSquareTile> {
  ...
  Widget build(BuildContext context) {
    final titleColor = widget.isDark ? AppTheme.white : AppTheme.lightTitle;
    final subColor = widget.isDark ? AppTheme.grey : AppTheme.lightSub;
    final bg = widget.isDark ? const Color(0xFF261118) : AppTheme.white;
...
class _PremiumBottomNavState extends State<PremiumBottomNav>
  ...
  Widget build(BuildContext context) {
    final fillColor = widget.isDark
        ? const Color(0xFF16070A).withValues(alpha: 0.82)
        : Colors.white.withValues(alpha: 0.80);
    final borderColor = widget.isDark
        ? AppTheme.primaryRed.withValues(alpha: 0.25)
        : AppTheme.lightAccent.withValues(alpha: 0.40);
    final glowColor = widget.isDark
        ? AppTheme.primaryRed.withValues(alpha: 0.18)
        : AppTheme.lightAccent.withValues(alpha: 0.14);
...
class GlowIconButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bgDark = accent
        ? AppTheme.surfaceLight.withValues(alpha: 0.4)
        : const Color(0xFF2D151B);
    final iconColor = accent
        ? (isDark ? AppTheme.primaryRed : AppTheme.lightAccent)
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [
                  iconColor.withValues(alpha: isDark ? 0.22 : 0.15),
                  iconColor.withValues(alpha: isDark ? 0.10 : 0.08),
                ],
                begin: Alignment.topLeft,
                end: Alignment.bottomRight,
              ),
