...
class TVProvider extends ChangeNotifier {
  ...
  @override
  void dispose() {
    unawaited(_discovery.stopScan());
    unawaited(_activeService?.disconnect());
    super.dispose();
  }
  ...
  Future<bool> sendKey(RemoteKey key) async {
    ...
    try {
      ...
      appLog('[TVProvider] Remote command failed: $error');
  ...
  Future<bool> sendRawKeyCode(int keyCode) async {
    ...
    if (service == null || !service.isConnected) return false;
    try {
      await service.sendRawKeyCode(keyCode);
      return true;
    } catch (error, stack) {
      appLog('[TVProvider] Raw remote command failed: $error');
      appLog(stack.toString());
      await service.disconnect();
      notifyListeners();
      return false;
  ...
  Future<bool> launchAppLink(String url) async {
    ...
    if (service == null || !service.isConnected) return false;
    try {
      await service.launchAppLink(url);
      return true;
    } catch (error, stack) {
      appLog('[TVProvider] App launch command failed: $error');
      appLog(stack.toString());
      await service.disconnect();
      notifyListeners();
      return false;
  ...
  final Set<String> _remoteUnlockedDevices = {};

  List<TVDevice> get recentDevices => List.unmodifiable(_recentDevices);

  /// Whether this TV has already passed the one-time rewarded unlock.
  bool isRemoteUnlocked(TVDevice device) =>
      _remoteUnlockedDevices.contains(device.id);

  /// Persist the one-time rewarded unlock for this TV. Unlocks are keyed by
  /// the device identity (brand + local IP), so reconnecting later does not
  /// show the reward again for the same TV.
  Future<void> markRemoteUnlocked(TVDevice device) async {
    if (!_remoteUnlockedDevices.add(device.id)) return;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(
      'tv_remote_unlocked_devices',
      _remoteUnlockedDevices.toList(),
    );
  }
  ...
  Future<void> _loadSavedTokens() async {
    final prefs = await SharedPreferences.getInstance();
    _remoteUnlockedDevices
      ..clear()
      ..addAll(prefs.getStringList('tv_remote_unlocked_devices') ?? const []);
