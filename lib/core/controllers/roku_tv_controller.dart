...
class RokuTVController implements TVController {
  ...
  Future<void> connect(String ip) async {
    ...
    appLog('[Roku] ${_connected ? 'Connected' : 'Unreachable'} at $_ip');
  }
  ...
  Future<void> sendKey(String keyName) async {
    ...
    appLog('[Roku] Unknown key: $keyName');
  }
  ...
  Future<void> _postKeypress(String rokuKey, String originalKey) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        appLog('[Roku] Keypress $originalKey → $rokuKey ✓');
      } else {
        appLog('[Roku] Keypress $rokuKey returned ${response.statusCode}');
      }
    } on Exception catch (e) {
      // Swallow timeouts / connection errors so the UI stays responsive.
      appLog('[Roku] Keypress $rokuKey failed: $e');
    }
  }
  ...
  Future<void> _postLaunch(String appId, String originalKey) async {
    ...
    try {
      final response = await http.post(uri).timeout(_requestTimeout);
      if (response.statusCode == 200) {
        appLog('[Roku] Launch $originalKey → appId $appId ✓');
      } else {
        appLog('[Roku] Launch appId $appId returned ${response.statusCode}');
      }
    } on Exception catch (e) {
      appLog('[Roku] Launch $originalKey failed: $e');
    }
  }
}
