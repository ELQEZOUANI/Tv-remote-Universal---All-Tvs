...
class DiscoveryService {
  ...
  Future<void> startScan({
    ...
    await _stopActiveScan(emitIdle: false);
    final generation = ++_scanGeneration;
    _seenIps.clear();

    // Start the watchdog before any permission or Bonjour work. Some native
    // discovery calls can stall, and the UI must never remain scanning forever.
    _scanTimer?.cancel();
    _scanTimer = Timer(timeout, () {
      if (generation == _scanGeneration) {
        _emitStatus(DiscoveryStatus.finished);
        unawaited(_stopActiveScan(emitIdle: false));
      }
    });
    ...
    for (final (type, brand, defaultPort) in _serviceTypes) {
      await _discover(type, brand, defaultPort, generation);
      if (generation != _scanGeneration) return;
    }

    // Start direct port probe immediately in background
    _emitStatus(DiscoveryStatus.probing);
    unawaited(_directSubnetProbe(generation));

    // The overall watchdog created above finalizes the scan at [timeout].
  }
  ...
  Future<void> _directSubnetProbe(int generation) async {
    final prefix = await NetworkHelper.getSubnetPrefix();
    if (prefix == null || generation != _scanGeneration) return;
    ...
    for (int start = 1; start < 255; start += batchSize) {
      ...
      for (int i = start; i < start + batchSize && i < 255; i++) {
        ...
        futures.add(_probeHost(targetIp, generation));
      }
  ...
  Future<void> _probeHost(String ip, int generation) async {
  ...
  Future<void> stopScan() async {
    await _stopActiveScan(emitIdle: true);
  }

  Future<void> _stopActiveScan({required bool emitIdle}) async {
    _scanGeneration++;
    _scanTimer?.cancel();
    _scanTimer = null;

    final discoveries = List<BonsoirDiscovery>.of(_discoveries);
    _discoveries.clear();
    ...
    _seenIps.clear();
    if (emitIdle) _emitStatus(DiscoveryStatus.idle);
  }
